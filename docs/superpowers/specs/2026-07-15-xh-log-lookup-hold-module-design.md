...
5. Loki 将授信/用信结果回调 `order`；成功则继续放款链路，失败时按解 H 状态、失败码和重路由结果判断是否再次进入 `HOLD_ON`。
6. 再次进入 H 单的订单等待后续解 H，形成“解 H → 推送 Loki → 失败回 H → 再解 H”的循环。
7. Hold 单取消、超时转单、特殊渠道处理或重路由。
8. Hold 单监控任务检查积压和异常。
...
- 进入 H 单、解 H、推送、Loki 结果回调、失败后再次进入 H 单、继续 Hold、取消/超时、重路由和监控的诊断步骤。
- 常见失败模式和状态判断规则。
- 无具体标识符时的健康检查步骤。
...
- Loki 失败回调：`LokiResultRequest OrderDataDTO is:`、`[新解H]放款失败，查询订单解h状态：`，并结合 `orderId`、`regCode` 和当前订单状态判断是否再次进入 H 单。
...
- 用户描述“解 H 推送后为什么又回到 H 单”：先确认 `[新解H推送]` 已成功推送，再查 Loki 的授信/用信失败回调；若日志显示 `releaseHoldStatus=Y` 后状态重置为 `HOLD_ON`，或失败 `regCode` 命中特殊回 H 配置，结论为放款结果触发再次进入 H 单。
...
- 区分两类回 H：推送 Loki 前因规则或额度不足继续 Hold；推送 Loki 后因授信/用信失败再次进入 H 单。两类场景的触发阶段和日志证据不同。
- Loki 返回失败不等于一定回 H。只有 `releaseHoldStatus=Y` 的解 H 订单在失败重置流程中恢复 `HOLD_ON`、失败码命中特殊回 H 配置，或重路由结果明确要求 Hold 时，才能下再次进入 H 单的结论。
...
基线应显示当前主路由只把“解H”指向 `loan.md`，且缺少进入 H 单、Loki 失败回调后再次进入 H 单、继续 Hold、取消/超时和监控链路。
...
- 能区分推送前继续 Hold 与 Loki 结果回调后再次进入 H 单，并给出相应查询锚点和判断条件。
