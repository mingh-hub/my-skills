...
为 `xh-log-lookup` 增加独立的 `references/modules/hold.md`，覆盖 Hold 单从进 H 到最终离开 Hold 状态的完整日志排查生命周期。模块必须能够指导按 `traceId`、`orderId` 或业务场景构造收窄后的 CLS 查询，并区分单笔链路排查与 Hold 服务健康检查。
...
- `order`：进 H 判断、Hold 状态维护、解 H 规则校验、资方/Loki 推送、继续 Hold、失败处理和重路由。
- `order-batch`：消费解 H 消息并推进订单处理。
...
2. 定时任务筛选可解 H 订单。
3. 解 H 消息进入 `order-batch` 消费链路。
4. 订单维度和资金维度执行解 H 校验。
5. 订单推送资方/Loki，或因规则、额度和状态原因继续 Hold。
6. Hold 单取消、超时转单、特殊渠道处理或重路由。
7. Hold 单监控任务检查积压和异常。
...
- 进 H、解 H、推送、继续 Hold、取消/超时、重路由和监控的诊断步骤。
- 常见失败模式和状态判断规则。
- 无具体标识符时的健康检查步骤。
- 与 `order.md`、`loan.md` 的边界说明。

推荐查询必须遵守主 skill 的 SQL 条件下沉规则。单笔查询优先组合 `serviceName`、稳定日志锚点和 `orderId`；`traceId` 按原值全链路查询，不拼 `serviceName`。统计或健康检查必须遵守完整性约束。
...
- 增加简短边界说明：Hold 单、进 H、解 H、继续 Hold、取消和超时问题读取 `references/modules/hold.md`。
...
- 进 H 判断：`非白名单校验结果`、`资产比例控制处理结果`，并结合订单号追踪进入 `HOLD_ON` 的原因。
- 解 H 任务：`[新解H]job开始`、`[新解H异常]`。
- 单笔解 H：`[新解H]订单:`、`订单维度解h校验结果`、`资金维度校验结果`、`进入解H状态`。
- 消息消费：`HandleHoldOrderConsumer推进来的消息ID`。
...
- 用户提供 `traceId`：首查 `traceId:"{value}"`，加载完整链路后识别跨服务节点。
- 用户提供 `orderId`：首查 `serviceName:"order" AND message:"{orderId}"`；若用户明确关注解 H、推送或取消，再加入对应稳定前缀收窄。
- 用户只描述“为什么还在 H 单”：依次检查进 H 原因、解 H 筛选、订单和资金校验、继续 Hold 原因、推送状态。
- 用户描述“解 H 失败”：检查 `[新解H异常]`、订单状态、可用资方、资金计划、额度、资产缓冲池和消息消费。
- 用户描述“Hold 单积压/异常”：按 `order`、`order-batch`、`order-batch-timing` 分服务执行健康检查，不用宽查询结果在本地筛选。
...
- MQ 消费缺失时，区分未发送、消费未发生和消费后失败，不能直接判断为订单侧失败。
...
基线应显示当前主路由只把“解H”指向 `loan.md`，且缺少进 H、消息消费、继续 Hold、取消/超时和监控链路。
