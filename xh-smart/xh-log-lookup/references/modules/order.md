...
无具体标识符时，先判断用户目标：**下单链路健康检查**主线是 `Step 1→2→3→4`；**订单服务/订单组健康检查**才按服务健康范围执行。`Step 0` 是订单服务通用 ERROR 背景检查，只能用于发现非 [借款下单] 类背景异常或补充风险，不得作为下单异常主统计。**健康检查属于统计模式，所有 Step 必须按主控 `xh-log-lookup` 的统计完整性规则执行：宿主 Agent 浏览器优先加载完整结果；仅切换到本地 Chrome 备用路径时才使用 `log_cls_query.py --require-complete --use-local-chrome`。**
