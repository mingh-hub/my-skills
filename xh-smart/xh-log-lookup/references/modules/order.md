...
订单模块 reference 只提供`首查策略`、`核心流程追踪入口`、`关键失败场景`、`健康检查`。CLS 执行、加载更多、飞书卡片优先输出和纯文本 fallback 统一交给 `xh-log-lookup` 主控工具。
...
- **申请前链路**：用户说"借款首页"、"借款内容"、"借款试算"、"预检"、"借款能力校验"、"首页不展示借款额度"、"客户申请借款"、"申请前轨迹"时，默认走 `references/modules/apply.md`；用户只说"借款失败"且未明确订单、下单或反欺诈时，也默认走 apply。只有确认已经进入正式下单、订单生成、下单拦截或反欺诈后，才回到本模块。
...
无具体标识符时，先判断用户目标：**下单链路健康检查**主线是 `Step 1→2→3→4`；**订单服务/订单组健康检查**才按服务健康范围执行。`Step 0` 是订单服务通用 ERROR 背景检查，只能用于发现非 [借款下单] 类背景异常或补充风险，不得作为下单异常主统计。**健康检查属于统计模式，所有 Step 必须按主控 `xh-log-lookup` 的统计完整性规则执行：宿主 Agent 浏览器优先加载完整结果；仅切换到本地 Chrome 备用路径时才使用 `cls_log_query.py --require-complete --use-local-chrome`。**
