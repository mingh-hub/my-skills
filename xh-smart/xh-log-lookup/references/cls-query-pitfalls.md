...
- Hermes 页面操作失败或需要批量自动提取时，切换到本地 Chrome 备用路径，用 `tools/cls_query.py --use-local-chrome --max-load-more` 自动点击“加载更多”。
- 提取全文后检查 `services` 覆盖。
...
1. 重新打开完整 CLS URL，优先使用 Hermes 内置浏览器。
...
如果 `log_count` 为 0 但全文中有日志内容，按采样结果分析，并在结论/飞书卡片里说明"总数未成功提取"。
