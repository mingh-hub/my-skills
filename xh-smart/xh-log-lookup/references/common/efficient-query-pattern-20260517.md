...
5. 当 WorkBuddy 页面操作失败、登录态不可用、或需要批量自动提取时，改用 `scripts/log_cls_query.py` 作为本地 Chrome 备用路径。
...
`scripts/log_cls_query.py` 默认只构造 URL。只有显式加 `--use-local-chrome` 时，才会复用本地 Chrome 备用窗口、自动点击“加载更多”并保存完整 `document.body.innerText`。只有切换到本地 Chrome 备用路径执行/提取时，统计模式才必须使用 `--require-complete --use-local-chrome` 并按工具返回的拆分建议处理不完整数据。
