...
|时间|问题描述|根因分析|解决方案|
|------|------|------|------|
|2026-06-02 14:51|私聊机器人，结论发到群聊|`messages-search API 的 --chat-type`指定先搜群组再搜个人，搜群组消息超时**模型**指定发到了**客户订单**群组内|查询去除`--chat-type`参数，根据返回消息中的`chat_type（group\|p2p）`判断是群组还是个人，执行对应的发送逻辑|
