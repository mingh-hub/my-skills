...
|借款校验|`/h5-loan/loan/loanCheck`|`[借款校验]cid`|<ul><li>入口日志：`serviceName:"h5-loan" AND message:"[借款校验]cid" AND message:"请求" AND message:"{cid}"`</li><li>出口日志：`serviceName:"h5-loan" AND message:"[借款校验]cid" AND message:"结果" AND message:"{cid}"`</li></ul>|**返回结果：**<ul><li>`notAvailBank`：卡是否不可用，`true`-不可用，`false`-可用</li><li>`supportBanks`：支持的银行卡列表</li></ul>|自营客户下单前的信息校验，包括`签约检查`，`银行卡限额校验`，`身份证状态检查`，`是否需要补充人脸`，`是否需要补充联系人`，`是否需要交易密码`，`查询是否需要学生声明`，`禁闭期查询`等业务，核心处理逻辑在`com.xhqb.h5loan.biz.service.service.check.userinfo.scene.LoanCheckStrategy#check`|
|借款下单|`/h5-loan/loan/loan`|`[借款申请]cid`|<ul><li>入口日志：`serviceName:"h5-loan" AND message:"[借款申请]cid" AND message:"请求" AND message:"{cid}"`</li><li>出口日志：`serviceName:"h5-loan" AND message:"[借款申请]cid" AND message:"结果" AND message:"{cid}"`</li></ul>|**请求入参：**<ul><li>`applyAmount`：借款金额</li><li>`applyStagesNbr`：借款期数</li><li>`applyCardNo`：借款申请银行卡号</li><li>`loanChannel`：借款渠道</li><li>`purpose`：借款目的</li><li>`channelOrderNo`：保费分期渠道订单号</li><li>`loanProcess`：借款流程</li><li>`loanProcessFlow`：0-老流程,1-新流程</li><li>`stagesChangeOrderId`：18期借款支持同时申请12期，上一笔允许变更期数的 orderId</li><li>`sceneValue`：1-仅转订单类型；2-即转期数又转订单类型</li></ul>**返回结果：**<ul><li>`customerId`：客户ID</li><li>`applyAmount`：下单成功金额</li><li>`orderPrice`：订单定价</li><li>`orderId`：下单成功的订单ID</li><li>`buyEquity`：是否购买权益，`true`-是，`false`-否</li><li>`ssyPageType`：试算页样式</li><li>`resultCode`：下单成功返回Code，`SUCCESS_RESPONSE`-下单成功</li></ul>|自营客户下单请求，`h5-loan`会调`order`下单的 dubbo 服务，`order`侧的下单业务参考`order.md`|
