...
|------|------|------|------|------|
|2026-06-04 21:01|群聊 @Tom 后卡片发到 HOME_CHANNEL 私聊|`send_feishu_card.py --resolve-chat` 只用完整 `source_query` 执行一次 `messages-search`；长文本混合 `traceId`、分支号、英文/数字/标点时飞书搜索分词可能返回 0 条，触发 `WORKBUDDY_HOME_CHANNEL_CHAT_ID` fallback|完整原始问题搜索 0 命中时，增加 `@Tom` 候选池 fallback：最近窗口最多拉 50 条 @Tom 群聊候选，仍校验 `mentions` 包含 Tom/BOT_OPEN_ID，再按原始消息相似度选择来源；低相似度、并列或无候选继续走现有 fallback|![alt text](image.png)|
